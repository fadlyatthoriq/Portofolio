...
include 'koneksi.php';
    ...
    $idgaleri = $_GET['id'];
    $sql = "SELECT * FROM galeri WHERE id='$idgaleri'";
    $query = mysqli_query($connect, $sql);
    $glr = mysqli_fetch_assoc($query);
        ...
        <form action="editgaleri.php" method="POST">
            <input type="hidden" name="id" value="<?php echo $glr['id']?>"/>
                <div class="form-group">
                    <label for="namaprofil">Gambar Galeri</label>
                    <input type="url" class="form-control" name="gambargaleri" value="<?php echo $glr['gambargaleri']?>"/>
                       </div>
                <div class="form-group">
                    <label for="gambarprofil">Teks Galeri</label>
                       <input type="text" class="form-control" name="teksgaleri" value="<?php echo $glr['teksgaleri']?>"/>
                       </div>
                <button class="btn btn-outline-primary mt-2" name="editgaleri" value="simpan">Simpan</button>
         </form>
    </div>
